...
void Application(void)
{
    ...
    NVIC_EN0_R |= (1 << 4);            // Enable interrupt 4 in the NVIC (GPIO Port E)
    ...
    unsigned char isComplete = 0;      // Flag indicating if the current input is complete
        ...
        if (intFlag)
        {
            ...
            else if (KeypadReading && !isComplete)
            {
                ...
                if (KeypadReading == '=' || inputArrayIndex > 15)
                {
                    ...
                    double res = calculateResultWithPriority();  // Calculate the result of the expression
...
double calculateResultWithPriority()
{
    unsigned char i = 0;
